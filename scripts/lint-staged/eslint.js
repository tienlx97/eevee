...
function groupFilesByPackage() {
  ...
  // use lerna for loop to get all package easy
  // just creaete lerna.json and add worspace inside
  const packagesWithEslint = Object.values(lernaAliases({ sourceDirectory: false })).filter(
    packagePath =>
      // exclude @eevee/noop (northstar packages root)
      path.basename(packagePath) !== 'eevee' &&

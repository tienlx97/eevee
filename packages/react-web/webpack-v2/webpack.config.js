...
const ForkTsCheckerWebpackPlugin1 =
...
const createEnvironmentHash = require('./persistentCache');
...
function loadOutput(isEnvDevelopment, isEnvProduction) {
  return {
    // The build folder.
    path: paths.appBuild,
...
function loadOptimization(isEnvProduction, isEnvProductionProfile) {
  return {
...
function loadResolve(isEnvProductionProfile) {
  return {
    fallback: {
...
function loadModuleRule(isEnvDevelopment, shouldUseReactRefresh, isEnvProduction, getStyleLoaders) {
  return {
...
function loadPluginList(isEnvProduction, env, isEnvDevelopment, shouldUseReactRefresh) {
  return [
    ...
    // useTypeScript && forkTsPlugin,

    useTypeScript &&
      new ForkTsCheckerWebpackPlugin1({
...
module.exports = function (webpackEnv) {
  ...
  return {
    ...
    output: loadOutput(isEnvDevelopment, isEnvProduction),
    ...
    optimization: loadOptimization(isEnvProduction, isEnvProductionProfile),

    resolve: loadResolve(isEnvProductionProfile),

    module: loadModuleRule(isEnvDevelopment, shouldUseReactRefresh, isEnvProduction, getStyleLoaders),

    plugins: loadPluginList(isEnvProduction, env, isEnvDevelopment, shouldUseReactRefresh),

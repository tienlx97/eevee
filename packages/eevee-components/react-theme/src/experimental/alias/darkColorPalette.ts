...
const statusColorPaletteTokens = (
  Object.keys(statusSharedColors) as Array<keyof Pick<GlobalSharedColors, StatusSharedColors>>
...
const personaColorPaletteTokens = (
  Object.keys(personaSharedColors) as Array<keyof Pick<GlobalSharedColors, PersonaSharedColors>>
).reduce((acc, sharedColor) => {
  const color = sharedColor.slice(0, 1).toUpperCase() + sharedColor.slice(1);
  const sharedColorTokens = {

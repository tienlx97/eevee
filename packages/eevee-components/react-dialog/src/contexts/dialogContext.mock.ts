...
/**
 * A test utility to mock the useDialogContext hook that relies on context selector
...
export const mockUseDialogContext = (options: Partial<DialogContextValue> = {}) => {
  ...
  (useDialogContext as jest.Mock).mockImplementation((selector: (context: DialogContextValue) => any) => {
